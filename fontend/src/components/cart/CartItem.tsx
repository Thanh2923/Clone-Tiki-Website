...
const CartItem: React.FC<handle> = ({handleSetTotal,handleCartItem}) => {
  ...
  return (
    ...
    {cart && cart.length > 0 ? <div className='w-full py-5 rounded-lg bg-white'>
        <div className="w-full  flex items-center px-5 py-2 justify-between">
          <div className="w-[50%]">
            <input type="checkbox" onChange={(e)=>handleCheckboxAll(e.target.checked)}  /> <span className="font-semibold text-slate-700">Tất cả ( <span className="text-red-500">{totalCheckBox ? totalCheckBox : 0} sản phẩm</span> )</span>
          </div>
          <span className="text-slate-500">Đơn giá</span>
          <span className="text-slate-500">Số lượng</span>
          <span className="text-slate-500">Thành tiền</span>
          <span onClick={()=>isOpen(null,IdCartItemChecked)} className=" cursor-pointer text-slate-500 text-xl"><MdDeleteOutline /></span>
        </div>
        <div className="w-full border-b-[1px] p-5">
         
          {cart.map(cart => (
            <div key={cart.id} className="product flex mb-3 justify-between items-center">
              ...
              <span onClick={()=>isOpen(cart.id,IdCartItemChecked)} className="font-semibold cursor-pointer text-lg text-slate-500"><MdDeleteOutline /></span>
            </div>
          ))}
        </div>
